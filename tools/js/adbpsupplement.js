...
// video auto play
function adbpAutoplayStore(clipId) {
	try { 
	    trackMetrics({ 
	        type: "track-autonext", 
	        data: {
	        	next_video_id: clipId
	        } 
	    }); 
	} catch(e){}
}

// video end slate click
...
var patterns = {
		...
		sf:[0,(/\.com\/(games|video|schedule|tv_shows|community|full-episodes)\/index\.html/),(/fusionfall\.cartoonnetwork\.com[\/]?(index\.php)?$/),(/nbahooptroop\.com[\/]?$/)
			,(/\/(around|games|leaders|scores|standings|tour|forums)[\/]?$/),(/blog\.cartoonnetwork\.com[\/]?$/),(/cartoonnetwork\.com\/blog[\/]?$/),(/mixels\.com[\/]?$/)],
		b:[0,(/blog\.cartoonnetwork\.com\//), (/cartoonnetwork\.com\/blog\//)],
		...
		g:[0,(/\/games/),(/\/tv_shows\/starwars\/games/),(/ben10gamecreator\.cartoonnetwork\.com/)
			...
			,(/\/mixit/)],
		v:[0,(/\/video/),(/\/ben10_video/),(/\/video\.htm/),(/cartoonnetwork\.com\/full-episodes/),(/\/livetv/)],
...
if (domain.indexOf("fusionfall") != -1) {
	...
	path1 = "starwarsgamecreator", path2 = "starwarsgamecreator";
} else if (pathname.match(/\/adventure-time-game-creator\//)) {
	path1 = "adventuretimegamecreator", path2 = "adventuretimegamecreator";
} else if (domain.indexOf("mixit") != -1) {
	if (path1) path2 = path1;
	path1 = "mixit";
} else if (domain.indexOf("mixels") != -1) {
	if (path1) path2 = path1;
	path1 = "mixels";
	...
	path1 = "blog";
	if (pathname.indexOf('post.jsp') > -1) {
		thisqs = location.search.split('='); 
		path2 = thisqs[1];
	} else {
		path2 = path_array[2]||"";
	}
...
} else if (pathname.match(/\/conqueror-of-all-worlds\//)) {
	path1 = "conqueror-of-all-worlds", path2 = "games";
} else if (pathname.match(/\/fusionfall-heroes\//)) {
	path1 = "fusionfall-heroes", path2 = "games";
...
/* super franchise */
var super_franchise = "";
if (pathname.match(/^\/(games|tv_shows)\/ben10/) || pathname.match(/^\/(games|tv_shows)\/ben-10-(.*)/)) super_franchise = "ben 10";
...
var pattern1 = {
	...
	"mixels":                           [(/\mixels\.com/),(/\staging\.mixels\.com/)],
	...
	"total drama pahkitew island":				[(/\/(games|tv_shows)\/total-drama/)],
	...
	"teen titans go":						[(/\/(games|tv_shows)\/titans/),(/\/(games|tv_shows)\/teen-titans-go/),(/\/(games|tv_shows)\/teentitans/)],
	...
	"uncle grandpa":				[(/\/(games|tv_shows)\/uncle-grandpa/)],
...
// assign a device type based on device name
var cn_adbp_devicetype = '';
if (typeof _cnglobal == "object") {
	switch (_cnglobal.device.deviceName) {
		case 'iPhone':
			cn_adbp_devicetype = 'phone';
			break;
		case 'iPad':
			cn_adbp_devicetype = 'tablet';
			break;
		case 'iPod':
			cn_adbp_devicetype = 'tablet';
			break;
...
var cn_adbp_playertype = '';
// check for CN instance of CVP and assign player type
if (typeof player == "object") {
	cn_adbp_playertype = player._playerType;
}
...
var cnt_metadata = {
	...
	player_type:			cn_adbp_playertype, 
	presentation_template:	cn_adbp_devicetype, 
